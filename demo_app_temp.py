...
_T = {
    ...
    "en": {
        ...
        "opt_contam": "Also run contamination analysis",
...
if "do_3d" not in st.session_state:
    st.session_state["do_3d"] = True
if "do_contam" not in st.session_state:
    st.session_state["do_contam"] = False
...
with st.expander("STEP 1.  설정 및 이미지 등록" if lang == "ko" else "STEP 1.  Settings & Image Uploads", expanded=True):
    ...
    with col_opt:
        ...
        do_contam = st.checkbox(T["opt_contam"], value=st.session_state["do_contam"])
        if do_contam != st.session_state["do_contam"]:
            st.session_state["do_contam"] = do_contam
            st.rerun()
    ...
    f_contam = _uploaded.get("f_contam") if do_contam else None
    f_3d = _uploaded.get("f_3d") if do_3d else None

    # 디폴트 로컬 이미지 자동 주입 레이어
    import io
    DEFAULT_PATHS = {
        "f_polar": "/Users/hyunchanan/Documents/GitHub/SG_integration_step1/test_image_260521/2B/2B_water.jpg",
        "f_nonpolar": "/Users/hyunchanan/Documents/GitHub/SG_integration_step1/test_image_260521/2B/2B_glycerol.jpg",
        "f_finish": "/Users/hyunchanan/Documents/GitHub/SG_integration_step1/test_image_260521/2B/2B_reflect.jpg",
        "f_3d": "/Users/hyunchanan/Documents/GitHub/SG_proj_007/example_01.jpg"
    }
    
    if f_polar is None and os.path.exists(DEFAULT_PATHS["f_polar"]):
        with open(DEFAULT_PATHS["f_polar"], "rb") as f:
            content = f.read()
            bio = io.BytesIO(content)
            bio.size = len(content)
            bio.name = os.path.basename(DEFAULT_PATHS["f_polar"])
            f_polar = bio
            
    if f_nonpolar is None and os.path.exists(DEFAULT_PATHS["f_nonpolar"]):
        with open(DEFAULT_PATHS["f_nonpolar"], "rb") as f:
            content = f.read()
            bio = io.BytesIO(content)
            bio.size = len(content)
            bio.name = os.path.basename(DEFAULT_PATHS["f_nonpolar"])
            f_nonpolar = bio

    if f_finish is None and os.path.exists(DEFAULT_PATHS["f_finish"]):
        with open(DEFAULT_PATHS["f_finish"], "rb") as f:
            content = f.read()
            bio = io.BytesIO(content)
            bio.size = len(content)
            bio.name = os.path.basename(DEFAULT_PATHS["f_finish"])
            f_finish = bio

    if do_3d and f_3d is None and os.path.exists(DEFAULT_PATHS["f_3d"]):
        with open(DEFAULT_PATHS["f_3d"], "rb") as f:
            content = f.read()
            bio = io.BytesIO(content)
            bio.size = len(content)
            bio.name = os.path.basename(DEFAULT_PATHS["f_3d"])
            f_3d = bio
...
def _load_engines():
    ...
    sam_w   = SAM2BaseWrapper(checkpoint_path="checkpoints/sam2_hiera_large.pt")
